...
perlperf - Perl Performance e Técnicas de Optimização

=head1 DESCRIÇÃO

Essa é uma introdução para o uso de performance e técnicas de optimização
que podem ser usadas como referencia para programas em Perl. Enquanto muitos 
desenvolvedores Perl vem de outras linguagens,  e podem usar seu conhecimento
aonde for apropriado, há um monte de outras pessoas que podem se beneficiar
de específicos pontos do Perl. Se você quer a versão condensada, talvez o melhor 
conselho venha do renomeado Japonês Samurai, Miyamoto Musashi, que diz:

	"Não se Envolva em Atividades Inúteis"

em 1645.

=head1 VISÃO POR CIMA

Talvez o erro mais comum que um programador comete é tentar optmizar o código
antes do programa fazer alguma coisa útil - essa é uma péssima idéia.
Não tem sentido ter um programa extremamente rápido que não funcione. A
primeira tarefa é pegar um programa que funcione I<corretamente> para fazer algo I<proveitoso>.
(sem falar garantindo que a suite de testes é completamente funcional)  e apenas então considerar
optimizar isso. Tendo decidido optmizar código em funcionamento, há 
algo importantemente simples mas essenciais passos para considerar que são intrínsecos para qualquer
processo de optimização.

=head2 UM PASSO PARA O LADO

Primeiro, você precisa estabelecer uma linha do tempo do código existente, que
o calendário precisa ser confiável e repetível. Você provavelmente quer usar os módulos
C<Benchmark> ou C<Devel::DProf>, or qualquer coisa similar, para este passo,
ou talvez o utilitário do sistema unix C<time>, conforme o caso. Veja a base
desse documento para uma lista mais completa de módulos de benchmarking e desempenho.

=head2 UM PASSO PARA FRENTE

Próximo, tendo examinado o programa por I<gargalos>, (lugares aonde o código
parece rodar mais devagar), mude o código com a intenção de fazer isso rodar rápido.
Usando software de controle de versão, como C<github>, irá assegurar que as
mudanças serão irrelevantes. Isso facilita para mexer aqui mexer ali - 
não mude muito por vez ou você talvez não irá descobrir que parte do 
código estatava B<realmente> lenta.

=head2 OUTRO PASSO PARA O LADO

Isso não é o suficiente para dizer: "isto vai fazer isso rodar rápido", você tem que checar isso.
reprisar o código sobre benchmarking ou sobre módulos de avaliação, vindo do
primeiro passo acima, e checar se esse novo código executa o B<mesmo código> em
I<menos tempo>. Salve o seu trabalho e repita...

=head1 ORIENTAÇÕES GERAIS

Uma coisa crítica quando consideramos performance é lembrar que não existe
coisas como a C<Bala Dourada>, e é por isso que não existem regras, apenas orientações.
É claro que o código inline vai ser mais rápido que uma sub-rotina ou chama de métodos,

porque código inline gera menos sobregarga, mas essa aproximação tem desvantagem
de ser menos mantível e vir a custar uma grande utilização de memória - 
não há coisas como almoço grátis. Se você está procurando por um elemento numa lista,
pode ser mais eficiente guardar os dados em uma estrutura de hash, e
então simplesmente olhar para ver se a chave está definida, ao invés do loop atravessar
todo o array usando grep() por exemplo. substr() pode ser (muito) mais rápido
do que grep() mas não tão flexível, então você tem outra trade-off para acessar. Seu
código pode conter linhas que demoram 0.01 segundos para executar, se você
chamar isso 1.000 vezes, como em um programa processando arquivos de médio porte
por exemplo, você já tenha um atrasado de 10 segundos, em apenas um único lugar
do código, e se você chamar essa linha 100,000 vezes, todo o seu programa
irá ficar lento como um verme rastejante.
