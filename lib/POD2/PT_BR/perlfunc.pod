...
Por exemplo:
        ...
        # Formatar número com 8 zeros sobrando no começo
        $result = sprintf("%08d", $number);

        # Arredondar número para 3 dígitos depois do ponto decimal
        $rounded = sprintf("%.3f", $number);

Perl faz sua própria formatação C<sprintf> -- emulando a função C C<sprintf>, 
mas a usa (exceto para números de ponto flutuante, e mesmo assim apenas os 
modificadores padrão são permitidos). Como resultado, qualquer extensão não 
padrão em seu C<sprintf> local não estão disponíveis para Perl. 

Diferente de C<printf>, C<sprintf> não faz o que provavelmente deseje quando 
você passa um array como seu primeiro argumento. O array é tratado em contexto 
escalar, e ao invés de utilizar o elemento 0 do array como um formato, Perl 
usará o quantidade de elementos no array como um formato, que quase nunca é 
útil. 

C<sprintf> do Perl permite as seguintes universalmente conhecidas converções:
   ...
   %e         um número ponto flutuante, em notação científica
   %f         um número ponto flutuante, em notação decimal fixa
   %g         um inteiro ponto flutuante, em notação %e ou %f

Em adição, Perl permite as seguintes largamente suportadas converções:
   ...
   %n         especial: *armazena* o número de caracteres emitidos na saída na próxima variável da lista de parâmetros

Finalmente, para retro (e nós queremos dizer "para trás") compatibilidade, Perl 
permite estas desnecessários mas largamente suportadas conversões.
...
C<%e>, C<%E>, C<%g> e C<%G> para números com os módulos do expoente menos que 
100 é dependente do sistema: pode ser três ou menos (preenchido com zeos se 
necessário). Em outras palavras, 1.23 vezes dez elevado a 99 pode ser "1.23e99" 
ou "1.23e099".

Entre o C<%> e a letra de formatação, você pode especificar um número de 
atributos adicionais controlando a interpretação do formato. Em ordem, são 
estes:
